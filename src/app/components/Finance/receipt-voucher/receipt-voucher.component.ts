...
GetAllSupAccounts(e:any){
  ...
  console.log("ok");
  console.log(e);
  this.MainAccpuntID=e.value
  let object=this.AllAccountsForRecieptAndPayments.filter((item: { mainId: any; })=>item.mainId==e.value)
  console.log(object);
  this.AllSupArr=[]
...
GetAllinsurerss(e:any){
  ...
  console.log(e.value);
  this._AccountsService.GetAllInsurerForAccount(e.value).subscribe((data:any)=>{
  ...
  AddPayment(){
    ...
    console.log(Model);
    this._PaymentsService.AddNewReciept(Model).subscribe(res=>{
      ...
      $(".HideCheck").hide(500)
      $(".chechBox").hide(500)
      this._Router.navigate(['/Payment/RecepitDetails']);
