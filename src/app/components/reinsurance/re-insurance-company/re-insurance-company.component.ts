...
export class ReInsuranceCompanyComponent implements OnInit{
  ...
  SubmitReInCompany(){
    this.isClicked= true;
    let Model = Object.assign(this.Form.value,{brokerOrReInsurance:0});
    console.log(Model);
    this._ReInsuranceService.AddNewReInsuranceCompany(Model).subscribe(res=>{
      this.isClicked= false;
      this.Form.reset();
      this._SharedService.setAlertMessage('Reinsurance Company Added Successfully');
      window.scrollTo({ top: 0, behavior: 'smooth' });
      // Swal.fire('Good job!','Reinsurance Company Added Successfully','success');
    },error=>{
      console.log(error);
      this.isClicked= false;
  ...
  ngOnInit(){
    this.getCountries()
    localStorage.setItem("act_Nav",'Reinsurance Company')
  }
}
